...
Animation::Animation(EntityStatus startStatus, AnimationFunction * s_function,
		EntityStatus endStatus, AnimationFunction * e_function, double duration,
		LoopMode loop) {
	duration_ = duration;
	loop_ = loop;
	__time = 0;
...
EntityStatus Animation::step(float dt) {
	if (!__end) {
		...
		float opacity = f->eval(__currentStatus.opacity, __nextStatus.opacity,
				dx);
...
void Animation::addListener(ActionListener * listener) {
